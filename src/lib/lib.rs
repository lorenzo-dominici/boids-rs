...
//! 
//! - `model`: Contains the implementation of the boids model in both Array of Structures (AoS) and Structure of Arrays (SoA) formats.
//! - `setup`: Contains the setup code for the simulation, including the configuration and context.
//! 
//! ## Usage
//! 
...
//! ## Example
//! 
//! Here is an example of how to use the library to run a simulation:
//! 
//! ```rust
//! 
//! use boids::{Config, Simulation};
...
//!     let rec = RecordingStream::new();
//! 
//!     // Create a new simulation with the configuration and recording stream
...
impl Simulation {
    ...
    pub fn from_config(config: Config, rec: RecordingStream) -> Self {
        Self { ctx: Context::new(config), rec }
    }
    ...
    pub fn run(&mut self) {
        ...
        self.rec.log("/logs", &TextLog::new(format!("Configs: {:?}", &self.ctx.config))).unwrap();
        ...
        for i in 1..=self.ctx.config.iters {
            ...
            // Sequential SoA
            self.ctx.soa_prev = self.ctx.soa_next.clone();
            t = time(|| self.ctx.soa_next.update(&self.ctx.soa_prev, &env));
            soa_t += t;
            self.log(&self.ctx.soa_next, soa_t, t, i, "soa_seq");

            // Sequential AoS
            self.ctx.aos_prev = self.ctx.aos_next.clone();
            t = time(|| self.ctx.aos_next.update(&self.ctx.aos_prev, &env));
            aos_t += t;
            self.log(&self.ctx.aos_next, aos_t, t, i, "aos_seq");

            // Parallel SoA
            let mut soa_par = self.ctx.soa_prev.clone();
            t = time(|| soa_par.par_update(&self.ctx.soa_prev, &env));
            soa_par_t += t;
            self.log(&soa_par, soa_par_t, t, i, "soa_par");

            // Parallel AoS
            let mut aos_par = self.ctx.aos_prev.clone();
            t = time(|| aos_par.par_update(&self.ctx.aos_prev, &env));
            aos_par_t += t;
            self.log(&aos_par, aos_par_t, t, i, "aos_par");
    ...
    fn log<B: BoidCollection>(&self, boids: &B, seconds: f64, duration: f64, iteration: u32, prefix: &str) {
        self.rec.set_time_seconds("seconds", seconds);
        self.rec.log(format!("/{}_duration", prefix), &Scalar::new(duration)).unwrap();
        self.rec.log(format!("/{}_average_duration", prefix), &Scalar::new(seconds / iteration as f64)).unwrap();
